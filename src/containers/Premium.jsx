...
const Premium = inject('store')(observer((props) => {
  ...
  const addToCart = (tariffId) => {
    actions.updateOrDeleteCart(tariffId);
  }

  const ticketsQuantityChanged = (tariffId) => {
    return selection[tariffId] !== cart[tariffId];
  }

  const ticketButtonLabel = (tariffId) => {
    if (!cart[tariffId])
      return 'Добавить в корзину';
    else if(!selection[tariffId] && cart[tariffId])
  ...
  return (
      <div>
        {tariffs.filter(simpleTariffsFilter).map(tariff => (
            <div className="premium_card" key={tariff.name}>
              <div className="premium_card_half">
                <div className="ticket_top">
                  <h2 className="ticket_title">{tariff.name}</h2>
                  <div className="ticket_desc ticket_desc_premium">
                    <img src="img/premium_img.png" alt="photo" className="premium_img"/>
                    <div className="premium_cabin_desc">
                      <div className="cabin_wrp">
                        <svg width="49" height="67" viewBox="0 0 49 67" fill="none"
                        ...
                        <span className="cabin">до 4-х пассажиров</span>
                      </div>
                      <span className="cabin_txt">Возможно совершить поездку туда-обратно без выхода для пересадки</span>
                    </div>
                  </div>
                </div>
                <div className="ticket_price ticket_price_premium">
                  <div className="ticket_price_col ticket_col_premium">
                    <img src="img/1side.svg" alt="" className="ticket_price_img"/>
                    <span className="number_txt">{tariff.session.price}₽</span>
                    <div className="main_number_plugin line" style={{width: 18}}>
                      <input type="text"
                             min={0}
                             max={100}
                             pattern="[0-9]*"
                             value={selection[tariff.session.id] || 0}
                             onChange={() => {}}
                             className="input_plugin_number"
                             style={{width: 18, height: 32}}/>
                      <div className="plus_plugin_number" onClick={() => actions.addToSelection(tariff.session.id)}/>
                      <div className="minus_plugin_number" onClick={() => actions.removeFromSelection(tariff.session.id)}/>
                    </div>
                  </div>
                </div>
                <button
                    className="btn ticket_btn"
                    disabled={!ticketsQuantityChanged(tariff.session.id)}
                    onClick={() => addToCart(tariff.session.id)}
                >
                  ...
                  <span>{ticketButtonLabel(tariff.session.id)}</span>
                </button>
              </div>
              <div className="premium_card_half">
                <p>
                  Кабины повышенного комфорта, где уютно даже в зимнюю стужу.
                  <br/><br/>
                  Вы сможете сделать полный круг,
                  взять с собой напитки из нашего
                  кафе, и совершить воздушную
                  прогулку, при желании не выходя
                  из кабины для пересадки, в
                  отличии от других пассажиров.
                  <br/><br/>
                  Идеально для желающих остаться
                  наедине, а также для семей до 4-х
                  человек.
                </p>
              </div>
            </div>
